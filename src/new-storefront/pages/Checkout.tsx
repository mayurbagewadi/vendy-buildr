...
const Checkout = ({ slug: slugProp }: CheckoutProps = {}) => {
  ...
  const mainShellClass = isEditorialCheckout
    ? "flex-1 bg-gradient-to-b from-[#fbfaf6] via-white to-[#f6f1e8] px-4 py-8 sm:px-6 lg:px-8"
    : "flex-1 container mx-auto px-4 py-8";
  const centerMainClass = isEditorialCheckout
    ? "flex-1 bg-gradient-to-b from-[#fbfaf6] via-white to-[#f5f1e8] px-4 py-16"
    : "flex-1 container mx-auto px-4 py-16";
  const checkoutInnerClass = isEditorialCheckout ? "mx-auto w-full max-w-7xl" : "";
  const breadcrumbClass = isEditorialCheckout
    ? "mb-8 flex items-center gap-2 text-sm text-stone-500"
    : "flex items-center gap-2 text-sm text-muted-foreground mb-6";
  const heroClass = isEditorialCheckout
    ? "mb-8 overflow-hidden rounded-2xl border border-stone-100 bg-white shadow-sm"
    : "";
  const heroInnerClass = isEditorialCheckout
    ? "bg-gradient-to-br from-white via-emerald-50/50 to-stone-50 p-6 sm:p-8"
    : "";
  const gridClass = isEditorialCheckout
    ? "grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_24rem]"
    : "grid grid-cols-1 lg:grid-cols-3 gap-8";
  const formColumnClass = isEditorialCheckout ? "lg:col-span-1" : "lg:col-span-2";
  const formStackClass = isEditorialCheckout ? "space-y-5" : "space-y-6";
  const ecoCardClass = isEditorialCheckout ? "overflow-hidden rounded-2xl border-stone-100 bg-white shadow-sm" : "";
  const ecoCardContentClass = isEditorialCheckout ? "p-5 sm:p-6" : "p-6";
  const ecoHeadingClass = isEditorialCheckout ? "mb-4 font-serif text-2xl font-semibold text-stone-950" : "text-xl font-semibold mb-4";
  const ecoSummaryCardClass = isEditorialCheckout ? "sticky top-24 rounded-2xl border-stone-100 bg-white shadow-xl shadow-stone-200/50" : "sticky top-24";
  const ecoMutedPanelClass = isEditorialCheckout ? "rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2" : "bg-accent p-3 rounded-lg";
    ...
    <div className={pageShellClass}>
      ...
      <main className={mainShellClass}>
        ...
        <nav className={breadcrumbClass}>
          <Link to={homeLink} className={isEditorialCheckout ? "hover:text-emerald-700" : "hover:text-foreground"}>Home</Link>
          <ChevronRight className="w-4 h-4" />
          <Link to={cartLink} className={isEditorialCheckout ? "hover:text-emerald-700" : "hover:text-foreground"}>Cart</Link>
          <ChevronRight className="w-4 h-4" />
          <span className={isEditorialCheckout ? "text-stone-900" : "text-foreground"}>Checkout</span>
        </nav>
        ...
        <div className={heroClass}>
          <div className={heroInnerClass}>
            ...
            <h1 data-ai="checkout-page-heading" className={isEditorialCheckout ? "font-serif text-4xl font-semibold leading-tight text-stone-950 sm:text-5xl" : "text-3xl font-bold mb-8"}>
              Checkout
            </h1>
            {isEditorialCheckout && (
        ...
        <div className={gridClass}>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className={formStackClass}>
                      ...
                      {locationEnabled && (
                        <div
                          ...
                          className={`space-y-3 rounded-xl border p-4 transition-all duration-300 ${
                            ...
                            locationError ? 'animate-attention-shake border-destructive bg-destructive/5 shadow-lg shadow-destructive/20' : (isEditorialCheckout ? 'border-stone-100 bg-stone-50/70 hover:border-emerald-200' : 'bg-card/50 backdrop-blur-sm hover:border-primary/50')
                          }`}
                        >
                ...
                <Card data-ai="payment-method-card" className={ecoCardClass}>
                  <CardContent className={ecoCardContentClass}>
                          ...
                          <div
                            ...
                            className={`relative cursor-pointer rounded-xl border-2 transition-all duration-300 overflow-hidden ${
                              selectedPaymentMethod === method.id
                                ? (isEditorialCheckout ? 'border-emerald-500 bg-gradient-to-br from-emerald-50 via-white to-stone-50 shadow-lg shadow-emerald-100/80' : 'border-primary bg-gradient-to-br from-primary/10 via-primary/5 to-transparent shadow-lg shadow-primary/20')
                                : (isEditorialCheckout ? 'border-stone-100 bg-white hover:border-emerald-200 hover:shadow-md' : 'border-border hover:border-primary/40 bg-card hover:shadow-md')
                            }`}
                    ...
                    {/* Payment info */}
                    {selectedPaymentMethod && (
                      <div className={isEditorialCheckout ? "mt-4 rounded-xl border border-emerald-100 bg-emerald-50 p-3" : "mt-4 p-3 bg-accent/50 rounded-lg border border-border"}>
                ...
                <Button
                  ...
                  type="submit"
                  size="lg"
                  className={isEditorialCheckout ? "min-h-[52px] w-full rounded-full bg-stone-900 px-8 text-sm font-black uppercase tracking-widest text-white hover:bg-emerald-800 md:w-auto" : "w-full md:w-auto"}
          ...
          <div data-ai="checkout-summary">
            <Card className={ecoSummaryCardClass}>
              <CardContent className={isEditorialCheckout ? "p-5 sm:p-6" : "p-6"}>
                <h2 data-ai="order-summary-heading" className={isEditorialCheckout ? "mb-1 font-serif text-2xl font-semibold text-stone-950" : "text-xl font-bold mb-4"}>Order Summary</h2>
                {isEditorialCheckout && (
                ...
                {/* Coupon Section */}
                <div data-ai="coupon-section" className={isEditorialCheckout ? "mb-4 space-y-3 border-b border-stone-100 pb-4" : "mb-4 pb-4 border-b border-border space-y-3"}>
                  <div className="flex items-center gap-2">
                ...
                <div className={isEditorialCheckout ? "mb-4 space-y-2 border-b border-stone-100 pb-4" : "space-y-2 mb-4 pb-4 border-b border-border"}>
