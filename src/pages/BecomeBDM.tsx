...
export default function BecomeBDM() {
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      console.log('[BecomeBDM] Starting Google sign up...');

      const redirectUrl = referralCode
        ? `${window.location.origin}/become-bdm?ref=${referralCode}`
        : `${window.location.origin}/become-bdm`;
      ...
      if (error) {
        console.error('[BecomeBDM] Google sign up error:', error);
        toast.error(error.message || "Failed to sign up with Google");
      } else {
        console.log('[BecomeBDM] Google OAuth redirect initiated');
      }
    } catch (error: any) {
      console.error('[BecomeBDM] Unexpected error:', error);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (storeWithEmail) {
        setBlockErrorMessage(`This email (${values.email}) is already registered as a store owner of "${storeWithEmail.name}". Store owners cannot become BDMs. Please use a different email.`);
      ...
      if (profileData) {
        ...
        if (existingStore) {
          setBlockErrorMessage(`This email is registered as store owner of "${existingStore.name}". Store owners cannot become BDMs. Please use a different email.`);
          setBlockModalOpen(true);
          return;
        }
      }
      ...
      if (storeWithPhone) {
        setBlockErrorMessage(`This phone number is already registered for store "${storeWithPhone.name}". Store owners cannot become BDMs. Please use a different phone number.`);
        setBlockModalOpen(true);
        return;
      }
      ...
      if (emailExists) {
        toast.error("This email is already registered as a BDM");
        return;
      }
      ...
      if (phoneExists) {
        toast.error("This phone number is already registered as a BDM");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Become a BDM — Earn Commission | DigitalDukandar</title>
        <meta name="description" content="Join DigitalDukandar's BDM program. Earn 10% commission helping Indian entrepreneurs build online stores. Apply free, work flexible hours." />
        <link rel="canonical" href="https://digitaldukandar.in/become-bdm" />
      </Helmet>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
            ...
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg">
              ...
              <p className="font-semibold">Build your own BDM network and earn additional 5%</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg">
      ...
      <div className="max-w-3xl mx-auto py-12 px-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Fill in your details to become a BDM</CardDescription>
          </CardHeader>
          <CardContent>
            {!isGoogleUser && (
              <div className="mb-6">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Why do you want to become a BDM?</FormLabel>
                        <FormControl>
                          <Textarea
                            placeholder="Tell us why you want to join our BDM program"
                            className="min-h-32"
                            {...field}
                          />
                        </FormControl>
