...
const WhatsAppFloat = ({ storeId }: WhatsAppFloatProps) => {
  ...
  return (
    <Button
      onClick={handleClick}
      className={`fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg hover:shadow-xl transition-none z-50 bg-transparent hover:bg-transparent p-0 border-0 overflow-hidden [&_svg]:!size-14${shouldHop ? ' animate-whatsapp-hop' : ''}`}
