...
const ReviewCarousel = ({
  ...
  // Mobile ticker auto-advance with seamless loop
  useEffect(() => {
    if (!isMobile || !autoPlay || reviews.length <= 1) return;
  ...
  }, [isMobile, autoPlay, autoPlayInterval, reviews.length]);

  // When ticker reaches the cloned first card, instantly jump back to real first card
  useEffect(() => {
    if (tickerIndex === reviews.length) {
      const timeout = setTimeout(() => {
        setIsTransitioning(false);
        setTickerIndex(0);
      }, 500); // Wait for slide animation to finish
      return () => clearTimeout(timeout);
    }
  }, [tickerIndex, reviews.length]);
  ...
  const renderCard = (review: Review, index: number) => (
    <Card
      ...
      className="review-card flex-shrink-0 border-0 shadow-md hover:shadow-lg transition-shadow"
    >
  ...
  // Mobile: Ticker-style (one card at a time, seamless infinite loop)
  if (isMobile) {
    // Append clone of first card at the end for seamless loop
    const tickerReviews = [...reviews, reviews[0]];
    ...
    return (
      <div className="w-full">
        <div className="review-ticker-container">
          <div
            ref={trackRef}
            className="review-ticker-track"
            style={{
              transform: `translateX(-${tickerIndex * 100}%)`,
              transition: isTransitioning ? "transform 0.5s ease-in-out" : "none",
            }}
          >
            {tickerReviews.map((review, index) => renderCard(review, index))}
          </div>
        </div>
            ...
            {reviews.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === (tickerIndex % reviews.length) ? "w-4 bg-primary" : "w-1.5 bg-gray-300 dark:bg-gray-600"
                }`}
              />
            ))}
          </div>
  ...
  // Desktop: Marquee scrolling (unchanged)
  const infiniteReviews = [...reviews, ...reviews];
  const scrollSpeed = 30;
  const totalDuration = scrollSpeed * 1000;
      ...
      <div className="review-marquee-container overflow-hidden">
        ...
        >
          {infiniteReviews.map((review, index) => (
            <Card
              key={`${index}-${review.author_name}`}
              className="review-card flex-shrink-0 sm:w-[calc(50%-0.5rem)] md:w-[calc(33.333%-0.667rem)] lg:w-[calc(20%-0.8rem)] border-0 shadow-md hover:shadow-lg transition-shadow"
            >
              <CardContent className="p-4 md:p-5 h-full flex flex-col">
                {/* Header: Name and Stars */}
                <div className="flex flex-col gap-2 mb-3">
